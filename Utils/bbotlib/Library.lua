	...
	local MenuName = isfile("lorix/menuname.txt") and readfile("lorix/menuname.txt") or nil
	...
	if not isfile("lorix/chatspam.txt") then --idk help the user out lol, prevent stupid errors --well it would kinda ig
		writefile(
			"lorix/chatspam.txt",
			[[
	WSUP FOOL
	...
	I LOVE LORIX YEAH
	PLACEHOLDER TEXT 
	dear lorix user, edit your chat spam
		]]
		)
	end

	if not isfile("lorix/killsay.txt") then
		writefile(
			"lorix/killsay.txt",
			[[
	WSUP FOOL [name]
	...
	PLACEHOLDER TEXT 
	dear lorix user, edit your kill say
		]]
		)
	end

	do
		local customtxt = readfile("lorix/chatspam.txt")
		for s in customtxt:gmatch("[^\n]+") do -- I'm Love String:Match
			table.insert(customChatSpam, s) -- I'm care
		end
		customtxt = readfile("lorix/killsay.txt")
		for s in customtxt:gmatch("[^\n]+") do -- I'm Love String:Match
	...
	if not isfolder("lorix") then
		makefolder("lorix")
		if not isfile("lorix/relations.lrx") then
			writefile("lorix/relations.lrx", "lrx:{{friends:}{priority:}")
		end
	else
		if not isfile("lorix/relations.lrx") then
			writefile("lorix/relations.lrx", "lrx:{{friends:}{priority:}")
	...
	local function GetConfigs()
		...
		if #result <= 0 then
			writefile("lorix/" .. menu.game .. "/Default.lrx", "")
		end
		return result
	end
	...
	local function UnpackRelations()
		local str = isfile("lorix/relations.lrx") and readfile("lorix/relations.lrx") or nil
		local final = {
			friends = {},
			priority = {},
		}
		if str then
			if str:find("lrx:{{") then
				writefile("lorix/relations.lrx", "friends:\npriority:")
				return
			end
	...
	local function WriteRelations()
		...
		writefile("lorix/relations.lrx", str)
	end
	CreateThread(function()
	...
	function menu.Initialize(menutable)
		local lrxmenu = {} -- this one is for the rendering n shi
		do
			Draw:MenuOutlinedRect(true, 0, 0, menu.w, menu.h, { 0, 0, 0, 255 }, lrxmenu) -- first gradent or whatever
			Draw:MenuOutlinedRect(true, 1, 1, menu.w - 2, menu.h - 2, { 20, 20, 20, 255 }, lrxmenu)
			Draw:MenuOutlinedRect(true, 2, 2, menu.w - 3, 1, { 127, 72, 163, 255 }, lrxmenu)
			table.insert(menu.clrs.norm, lrxmenu[#lrxmenu])
			Draw:MenuOutlinedRect(true, 2, 3, menu.w - 3, 1, { 87, 32, 123, 255 }, lrxmenu)
			table.insert(menu.clrs.dark, lrxmenu[#lrxmenu])
			Draw:MenuOutlinedRect(true, 2, 4, menu.w - 3, 1, { 20, 20, 20, 255 }, lrxmenu)

			for i = 0, 19 do
				Draw:MenuFilledRect(true, 2, 5 + i, menu.w - 4, 1, { 20, 20, 20, 255 }, lrxmenu)
				lrxmenu[6 + i].Color = ColorRange(
					i,
					{ [1] = { start = 0, color = RGB(50, 50, 50) }, [2] = { start = 20, color = RGB(35, 35, 35) } }
				)
			end
			Draw:MenuFilledRect(true, 2, 25, menu.w - 4, menu.h - 27, { 35, 35, 35, 255 }, lrxmenu)

			Draw:MenuBigText(MenuName or "Bitch Bot", true, false, 6, 6, lrxmenu)

			Draw:MenuOutlinedRect(true, 8, 22, menu.w - 16, menu.h - 30, { 0, 0, 0, 255 }, lrxmenu) -- all this shit does the 2nd gradent
			Draw:MenuOutlinedRect(true, 9, 23, menu.w - 18, menu.h - 32, { 20, 20, 20, 255 }, lrxmenu)
			Draw:MenuOutlinedRect(true, 10, 24, menu.w - 19, 1, { 127, 72, 163, 255 }, lrxmenu)
			table.insert(menu.clrs.norm, lrxmenu[#lrxmenu])
			Draw:MenuOutlinedRect(true, 10, 25, menu.w - 19, 1, { 87, 32, 123, 255 }, lrxmenu)
			table.insert(menu.clrs.dark, lrxmenu[#lrxmenu])
			Draw:MenuOutlinedRect(true, 10, 26, menu.w - 19, 1, { 20, 20, 20, 255 }, lrxmenu)

			for i = 0, 14 do
				Draw:MenuFilledRect(true, 10, 27 + (i * 2), menu.w - 20, 2, { 45, 45, 45, 255 }, lrxmenu)
				lrxmenu[#lrxmenu].Color = ColorRange(
					i,
					{ [1] = { start = 0, color = RGB(50, 50, 50) }, [2] = { start = 15, color = RGB(35, 35, 35) } }
				)
			end
			Draw:MenuFilledRect(true, 10, 57, menu.w - 20, menu.h - 67, { 35, 35, 35, 255 }, lrxmenu)
		end
		...
		for k, v in pairs(menutable) do
			...
			table.insert(tabs, { lrxmenu[#lrxmenu - 2], lrxmenu[#lrxmenu - 1], lrxmenu[#lrxmenu] })
		...
		Draw:MenuOutlinedRect(true, 10, 59, menu.w - 20, menu.h - 69, { 20, 20, 20, 255 }, lrxmenu)

		Draw:MenuOutlinedRect(true, 11, 58, ((menu.w - 20) / #menutable) - 2, 2, { 35, 35, 35, 255 }, lrxmenu)
		local barguy = { lrxmenu[#lrxmenu], menu.postable[#menu.postable] }
		...
		function menu.saveconfig()
			...
			CreateNotification('Saved "' .. menu.options["Settings"]["Configuration"]["ConfigName"][1] .. '.lrx"!')
			UpdateConfigs()
		end
		
		function menu.loadconfig()
			local configname = "lorix/"
				.. menu.game
				.. "/"
				.. menu.options["Settings"]["Configuration"]["ConfigName"][1]
				.. ".lrx"
			...
			if pcall(LoadConfig, loadedcfg) then
				CreateNotification('Loaded "' .. menu.options["Settings"]["Configuration"]["ConfigName"][1] .. '.lrx"!')
			else
				LoadConfig(curcfg)
				CreateNotification(
		...
		local function buttonpressed(bp)
			...
			elseif bp == menu.options["Settings"]["Configuration"]["Delete Config"] then
				...
				CreateNotification('Deleted "' .. menu.options["Settings"]["Configuration"]["ConfigName"][1] .. '.lrx"!')
				UpdateConfigs()
	...
	do
		...
		wm.textString = " | " .. LORIX.username .. " | " .. os.date("%b. %d, %Y")
